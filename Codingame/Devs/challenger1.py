"""
    ...
    La fonction initialise_game() est chargée de récupérer les données initiales du jeu, notamment le nombre de poissons, leurs caractéristiques (couleur, type), ainsi que les informations sur les drones (position, batterie, etc.).
    Mise à jour des informations :

    Les fonctions comme initialise_loop() sont responsables de mettre à jour les données à chaque tour de jeu. Elles récupèrent les scans des poissons, l'état des drones (position, batterie, scans effectués), les scores, etc. Ces informations serviront à prendre des décisions pour le tour suivant.
    Calcul des scores :

    La fonction score_for_scan(scans:List[int])->int calcule un score en fonction des scans effectués par les drones. Ce score détermine la valeur potentielle de l'exploration d'une zone contenant des poissons non encore scannés par l'adversaire.
    Déplacement des drones :

    Plusieurs stratégies de déplacement sont mises en œuvre :
    Déplacement vers une cible : La fonction next_move() calcule le prochain déplacement d'un drone vers une cible en fonction de sa vitesse maximale.
    Évitement des bords de la carte : La fonction edge_move() ajuste le déplacement d'un drone pour éviter les bords de la carte, assurant ainsi qu'il ne quitte pas la zone de jeu.
    Chasse des poissons : Une stratégie de chasse est mise en œuvre pour déplacer les drones vers les poissons, en particulier pour chasser les poissons qui se dirigent vers les bords de la carte ou vers des positions défavorables.
    Surface pour récupération : La fonction surface_value() évalue l'avantage de remonter à la surface pour récupérer des poissons et détermine si cette action est bénéfique.
    Détection des obstacles :

    La fonction monsters_in_way() vérifie la présence de monstres sur le chemin d'un drone et ajuste son déplacement en conséquence pour éviter les collisions.
    Stratégies spécifiques :

    Des stratégies spécifiques sont implémentées pour les monstres et les poissons, notamment pour ajuster leur comportement en fonction de leur statut (agressif, endormi, etc.).
    Analyse de l'état du jeu :

    La fonction show_game_state() fournit des informations sur l'état du jeu à chaque tour, ce qui peut aider à prendre des décisions stratégiques.
    Calcul des mouvements optimaux :

    Des fonctions telles que closest_target() et closest_fish_dist() calculent la cible la plus proche ou la distance au poisson le plus proche, ce qui aide à déterminer les actions à entreprendre pour chaque drone.
"""
