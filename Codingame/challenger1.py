"""
    Cette logique calcule la distance entre chaque drone et chaque poisson visible.
    Ensuite, elle sélectionne le poisson le plus proche et déplace le drone vers sa position.
    Si aucun poisson n'est visible, le drone reste à sa position actuelle.
"""
...
while True:
    ...
    # Logique de déplacement des drones
    for drone in my_drones:
        min_distance = float('inf')  # initialisation avec une distance infinie
        closest_fish = None
        
        for fish in visible_fish:
            dist = distance(drone.pos, fish.pos)
            if dist < min_distance:
                min_distance = dist
                closest_fish = fish
        
        if closest_fish:
            target_x = closest_fish.pos.x
            target_y = closest_fish.pos.y
        else:
            # Si aucun poisson n'est visible, le drone reste immobile
            target_x = drone.pos.x
            target_y = drone.pos.y
        
        light = 1  # Lumière par défaut
